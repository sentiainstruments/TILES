...
- **Melodic mode: live echo of an incoming melody.** Real feedback: "in
  ...
  flat 128-entry `s_incoming_note_sounding[]` -- any MIDI channel (what
  feeds this board's MIDI IN is the TILES DISPLAY Max for Live device,
  `daw-integration/ableton/TILES_DISPLAY/`, see the next paragraph;
  nothing here configures or assumes a channel), and
  ...
  **Real feedback on the first version's setup, and the device that
  replaced it**: "the thing is that ableton instruments send either midi
  or audio after the vst, can we auto do both? if not then we need to
  build a max for live device that slots in between the device must have
  an arm button to display on tiles and if multiple instances are on
  different tracks then it should disarm from previous if possible...
  we can call it 'VIEW' and make it sentia style... the button is sentia
  pink when armed. the plugin is called TILES DISPLAY." The first
  version's DAW-side instruction (point the armed track's MIDI output at
  TILES) only ever worked for a track with NO instrument: after an
  instrument, a track's output is audio. "Auto both" isn't possible --
  a Remote Script only sees its own MIDI port, never a track's MIDI, and
  reading a playing clip's notes from Python would only cover clip
  playback, not live-played notes or anything an arp/chord device
  upstream produces. So: **`daw-integration/ableton/TILES_DISPLAY/`**, a
  Max for Live MIDI Effect that sits before the instrument (sees every
  note that reaches it, passes all of it through untouched) and sends
  each note to TILES through the Live Object Model's `ControlSurface.
  send_midi` -- which writes to the TILES control surface's own MIDI
  output port, the same one `scene_launch.py` already sends SysEx
  feedback through (confirmed against Cycling '74's LOM docs and forum
  reports of the same technique lighting Push pads) -- so the firmware
  parser above needed no changes at all; Note-On velocity 0 is already
  its Note-Off. Firmware side unchanged; see `daw-integration/README.md`
  for install/use, the VIEW toggle (Sentia pink armed, exclusive across
  instances via Max's global name space), and the SURFACE number.
  **First real run, read from Live's own `Log.txt`**: nothing lit, and the
  log showed exactly why -- the whole tap/gate/`send_midi` chain worked
  (`call send_midi 144 88 127` issued correctly for every note) but every
  call was rejected with `no valid object set`. Root cause was this
  section's own instruction: SURFACE was documented as the Preferences
  slot number, but Live's device bridge (`_MxDCore/LomTypes.py`,
  disassembled from the installed app) builds the `control_surfaces N`
  list with `filter(lambda c: c is not None, application.control_surfaces)`
  -- empty slots are skipped, so N counts *loaded* scripts. The user's
  real layout (Launchkey_MK4, MPK_mini_mkIII, TILES in slots 1-3 of 7;
  the first two with no ports set) made "slot 3" point at nothing. Fixed
  the docs, widened the range to 1-7 (Live 12.4 has 7 slots, not 6), and
  made the device flash a run of pads on TILES whenever SURFACE changes
  or VIEW arms, so finding the right number is stepping it until pads
  light rather than guessing -- the LOM offers a device no way to ask a
  control surface which script it is (only `pad_layout` plus functions).
  **Confirmed working on real hardware**, then two follow-ups from the
  same message: "its workig! it needs more brightness tho, also can it
  override screensavers ? like no screensaver can activate if ableton is
  playing or midi is beinmg recived ?"
  - *Brightness.* The echo color was already full-scale (0, 1, 0) in
    `lighting.c`'s own 0-1 space, so there was nothing left to raise
    there: the limit is `write_pad()`'s power-derived per-pad ceiling
    (`static_ceiling_level()` -- 37% on USB-only, 90% on external power),
    which exists for a documented safety reason (unmeasured haptics
    current against a 500mA USB budget; "never auto-select a USB power
    budget above 500mA" is a hardware non-negotiable) and was
    deliberately left alone. What can change within the same ceiling is
    how much light a pad emits at it: a pure-green pad drives only the
    green die, so the sustained color now mixes in a little red/blue
    (`TILES_LIGHTING_ECHO_SUSTAIN_TINT`, a lighter green), and every note
    lands with an onset flash -- full white easing down to that green over
    `TILES_LIGHTING_ECHO_FLASH_MS` (180ms), driven by a new
    `tiles_op_mode_incoming_note_age_ms()` fed from a per-note
    `s_incoming_note_on_ms[]` recorded in the Note-On callback. White at
    the ceiling is what a pressed pad already shows, so it stays inside
    the budget the ceiling was built around. Worth knowing: on USB-only
    power every pad on this board is capped at 37%, so external power is
    the single biggest brightness lever there is.
  - *Screensaver.* `services/standby.c` decides idle from
    `real_input_active()` (touch/buttons/pedal) plus a menu-open
    hold-off; it had no idea MIDI existed. `midi/midi_in.c` now keeps
    `tiles_midi_in_activity_count()`, bumped by every Note-On/Off and
    every Real-Time Clock/Start/Continue/Stop -- deliberately NOT SysEx
    (Scene Launch feedback fires on any clip change, playing or not), so
    "MIDI received" means the board is actually being used rather than
    "Ableton is open." `tiles_standby_scan()` compares it against the
    last value it saw: while AWAKE, activity refreshes the idle timer
    exactly like a touch (external clock is ~48 events/s, so a playing
    DAW holds the screensaver off continuously; a lone note holds it off
    for one more idle timeout; sequencer mode's own 20-minute timeout is
    covered too -- a pattern running off Ableton's clock is precisely the
    unattended case). An AUTOMATIC screensaver or deep sleep that's
    already showing wakes on it, since it would otherwise hide the very
    melody or synced pattern the MIDI is driving. A MANUAL one -- the
    player held circle ~4s (screensaver, `s_manual_screensaver`) or ~8s
    (deep sleep, new `s_deep_sleep_manual`, since `enter_deep_sleep()`
    is shared with the automatic path and clears the other flag) -- is
    left alone entirely: a deliberate request must not be undone by a
    DAW that happens to be playing, and MIDI can't keep pushing its
    inactivity timeout back either. No `printf` on that wake path
    (`print_wake_source()` only knows touch/button/pedal, and this runs
    at clock rate) -- this project's own history with USB-CDC stdio
    blocking the main loop.
  - *Two devices at once (this superseded "exclusive across instances").*
    Real feedback: "make the device work on 2 channels at once, if 2
    devices are on then the secondary does color red." Firmware: the note
    callback now keeps TWO layers (`TILES_OP_MODE_ECHO_LAYERS`), keyed by
    MIDI channel -- channel 2 is layer 1 (the secondary), every other
    channel is layer 0 (the primary, unchanged), each with its own
    `s_incoming_note_sounding[layer][128]` / `s_incoming_note_on_ms[layer]
    [128]`, so a Note-Off from one device can never clear a same-pitch
    ...
    lone armed device is never red. That arbitration is plain Max
    send/receive between instances with no stored ids (a deleted-while-
    armed device can't wedge a slot). Since Live can't be driven from
    here, the generated patch was run through a small Max-semantics
    simulator (three instances, arm/bump/promote/deleted-instance
    scenarios, 300 random fan-out orderings) -- all pass; that proves
    the protocol, not that Live loads the file, which is still the
    first-run checklist's job.
  Honest status: the device is generated by `build_tiles_display.py`
  (same container format as Ableton's own factory templates, with every
