...
- **Sequencer patterns now survive a power cycle -- the first real
  persistent storage in this codebase.** Real feedback: "add memory of
  last session when powered off for the sequencers." `firmware/src/
  storage/` existed only as a design README before this (`storage/
  README.md`'s own "versioned, CRC-protected... two alternating slots"
  plan) -- `storage/seq_store.h`/`.c` is the first real implementation of
  that pattern, scoped narrowly to the sequencer's own data rather than
  the README's full eventual scope (calibration/profiles/CV/LED/motor
  config etc. all stay unimplemented, deliberately not pulled in here).
  **Not a true "save at the instant power is cut."** This board has no
  brownout/VSYS-loss detection (services/power.h only ever reports WHICH
  source is currently feeding it, never an early warning that all of
  them are about to disappear) and no energy reserve (supercap/battery)
  to keep the MCU alive long enough to finish a flash write after power
  actually goes away -- not reliably achievable on this hardware at all,
  and attempting one anyway risks starting a write that then gets cut
  mid-erase, a real corruption risk this design specifically avoids.
  Instead: op_mode.c auto-saves a few seconds after the last sequencer
  edit goes quiet (`seq_store_mark_dirty()`, called from every mutation
  site -- arm-toggle, length-adjust, pitch/probability/ratchet commits,
  pattern-bank switches, per-pattern scale commits; debounced 3s,
  checked unconditionally every scan in `tiles_op_mode_scan()`, ahead of
  every other branch so a due save never waits on which view is
  displayed). By the time real power loss happens, flash already
  reflects whatever was last edited -- the same way most consumer gear
  with "remembers your last session" actually works.
  **Flash layout**: the LAST TWO 4KB sectors of the Pico 2's 4MB flash
  (`PICO_FLASH_SIZE_BYTES - 2*FLASH_SECTOR_SIZE` / `- 1*FLASH_SECTOR_
  SIZE`), alternating on every save -- an interrupted/failed write can
  only ever corrupt the slot being WRITTEN, never the other one, which
  stays loadable regardless. Each slot: a magic number, a format
  version, a monotonically increasing sequence number (whichever slot
  has the higher one wins at load), the actual pattern data, and a
  CRC32 over all of it -- a slot that fails ANY of those checks (wrong
  magic/version, or a CRC mismatch from a torn write) is treated
  exactly like "nothing saved yet," never trusted partially. Comfortably
  far from the firmware image itself (currently ~200KB, of 4MB total),
  so normal `picotool load` reflashing during development doesn't touch
  it either -- confirmed by reading `picotool load --help`'s own
  description ("load the program / memory range stored in a file," not
  a full-chip erase) rather than assumed.
  **Why a debounced auto-save is safe here specifically**: a flash
  erase+program briefly disables interrupts (hardware_flash's own
  requirement, since the CPU can't fetch code via XIP -- which is where
  this firmware executes from -- while flash is mid-write). This project
  is single-core and services USB via polled `tud_task()` calls from the
  main loop rather than an interrupt handler (see main.c's own header
  for why), so a brief disabled-interrupts window is nowhere near the
  same risk class as, say, an interrupt-driven USB stack silently
  dropping a transfer -- worst case is a single main-loop iteration
  taking ~10-50ms longer, and it only ever happens after 3+ seconds of
  no editing, i.e. precisely when nothing time-sensitive is happening.
  Deliberately NOT wired into the per-note/per-edit hot path for the
  identical reason the haptics printf freeze (this file's own entry
  above) needed fixing at all -- anything synchronous and blocking in
  that path is the exact failure mode already found and fixed once this
  session.
  **On-disk format decoupled from op_mode.c's own internal representation**
  on purpose: `tiles_seq_store_pattern_t`/`tiles_seq_store_data_t` are a
  separate, explicitly-laid-out mirror of `op_seq_pattern_t` and the
  per-lane channel/active-alternative arrays, with op_mode.c owning the
  conversion both ways (`seq_store_pattern_to_wire()`/`_from_wire()`) --
  storage/ itself never references op_mode.c's own types, matching this
  codebase's established pattern of keeping lower-level services
  decoupled from any one consumer's internals. `_Static_assert`s tie the
  two files' lane/alternative/step-count constants together at build
  time (a mismatch fails the BUILD, not silently truncates data at
  runtime), and defensive clamps on load (`length`, `scale`, `active_
...
- **Scale picker exit was breaking sequencer mode's own view -- same bug
  ...
  comment).
- **Per-pattern scale picker was write-only -- picking a scale for a
  pattern never actually changed what note a step played.** Real
  feedback: "good but patternm mode wont lock to selectesd scales on
  scsale selector it stays in chromsatic." Root cause: `scale_menu_exit()`
  (this file's own entry just above) correctly stores the picked scale
  into `active_pattern()->scale` on close, then restores the real GLOBAL
  scale back to whatever it was before the picker temporarily swapped it
  (`scale_menu_enter(true)`'s own swap-in/swap-out design) -- but every
  site that resolves a step's actual pitch (`seq_handle_step_taps()`'s
  arm-toggle, and the hold-to-assign commit in `handle_edit_mode()`) was
  still calling plain `tiles_note_map_get_note(pad)`, which only ever
  reads THAT live global scale. The per-pattern value got written, never
  read back -- so a step armed or reassigned after the picker closed
  silently used whatever scale happened to be global at that moment
  (chromatic by default), exactly the reported symptom.
  **Fix**: new `tiles_note_map_get_note_in_scale(uint8_t logical_pad,
  tiles_scale_mode_t scale)` in `note_map.h`/`.c`, taking the scale as an
  explicit argument instead of trusting the global. Deliberately narrower
  than `tiles_note_map_get_note()` -- no guitar-mode/chord-mode special
  casing -- since sequencer mode is never simultaneously guitar or chord
  mode, so that logic has nothing to do here. `current_scale_table()`'s
  own chromatic fallback (an unrecognized/uninitialized scale silently
  becomes `TILES_SCALE_CHROMATIC` rather than an empty table) is shared
  via a new small `scale_table_with_fallback(scale)` helper, so both the
  global lookup and this new explicit-scale lookup get the identical
  safety net. Four call sites in `op_mode.c` now resolve through this
  pattern-scoped function instead: the arm-toggle and hold-to-assign
  commits (the two places a step's pitch is actually DECIDED), plus
  `seq_fire_note()`'s own not-yet-overridden fallback and `render_pitch_
  edit()`'s fallback/per-pad comparison, kept consistent for the same
  reason even though the first is effectively unreachable in practice
  (arm-toggle always sets `step_pitch_override` true the instant a step
  is armed) and the second is purely visual -- without it, the picker's
  white "currently assigned" pulse would highlight the wrong pad (or
  none) the moment a pattern's own scale differs from the global one,
  since it would still be comparing against a global-scale-resolved note.
  **Deliberately left alone**: `seq_capture_handle_taps()` still reads
  the plain global-scale `tiles_note_map_get_note()`, correctly --
  `seq_capture_mode_enter()` swaps the global scale to `TILES_SCALE_
  CHROMATIC` for its entire duration by design (live capture performance
  is always full chromatic, independent of any pattern's own stored
  scale), so that site was never part of this bug. The pitch-assign
  picker's decorative root/natural/sharp background coloring (`tiles_
  note_map_is_root_pad()`/`_is_natural_pad()`) also still reflects the
  global scale rather than the pattern's own -- left as-is since it's
  cosmetic only (the picker already lets you tap ANY pad regardless of
  that coloring, a full chromatic override, so nothing is gated on it),
  and neither of those helpers currently takes a scale argument; adding
  that pair for a purely decorative mismatch wasn't justified by the
  actual reported defect.
- **Second real-hardware freeze, root cause this time NOT sequencer-
  specific -- a whole class of unconditional "temporary bring-up"
  printf() calls scattered across several files, all sharing the exact
  USB-CDC blocking hazard this file's own haptics.c entry above already
  named.** Real feedback: "froze again with enough time." Unlike the
  haptics.c round (needed capture mode plus a dense external clock),
  this one didn't need anything specific to be happening -- "enough
  time" was the whole trigger, which pointed straight at `main.c`'s main
  loop rather than any one feature:
  **`main.c`'s periodic 2-second bring-up dump** (`tiles_diag_i2c_scan_
  expected_devices()` -- 9 printf calls, one per expected I2C device
  plus a summary -- plus 3 more for power/Hall/standby state, 12 total)
  ran unconditionally on a plain wall-clock timer, forever, from the
  moment the device powered on, whether or not a serial terminal was
  ever attached to drain the CDC buffer. With no terminal open (the
  normal case once TILES is plugged into a DAW/synth rig instead of a
  dev machine), the buffer fills and every printf() call blocks up to
  `PICO_STDIO_USB_STDOUT_TIMEOUT_US` (500ms) -- up to 12 * 500ms = 6s of
  main-loop stall possible every 2s once the buffer settled into
  "always full." This is the only one of the culprits found this round
  that's fully activity-independent -- it explains a freeze regardless
  of mode or what the player was doing, purely from elapsed power-on
  time, matching the report exactly. Removed outright (the whole
  `last_scan_ms`/2-second-timer block, not just the prints inside it --
  dead scaffolding otherwise), along with the timer variable that
  existed only to drive it. The one-shot i2c scan still called once at
  boot (`tiles_op_mode_init()`'s own neighborhood) is untouched -- fires
  once before the loop starts, not on a recurring timer, so it can't
  accumulate.
  **`services/touch.c`'s per-touch-edge print** fired on every touch AND
  release of every pad -- a normal event during any play at all
  (melodic notes, chord notes, sequencer step arming), not a rare one.
  **`services/expression.c` had three more**: one on every committed
  note-on (fires on every single note played in melodic/chord mode),
  one on every touch that ended without ever registering a real press
  (also common -- light taps happen constantly during real playing),
  and one firing whenever a held pad's live pitch-bend value changed
  from the last value actually sent -- during genuine expressive play
  (holding a pad while varying pressure/tilt, this instrument's whole
  point), the computed bend changes on very close to every scan
  iteration, making this print the closest thing in the whole codebase
  to truly continuous/unthrottled -- a real, likely dominant contributor
  any time pitch bend was in active use for a while. A fourth print in
  the same function, `[depth-cal]`, was already throttled to at most
  once per 150ms after an EARLIER pitch-bend freeze in this same file's
  history (see that print's own now-removed comment: "150ms here is
  deliberately conservative... caused a real firmware freeze earlier
  this same debugging arc") -- still a real risk on its own (12s worth
  of possible blocking per 150ms-interval print, if every single one
  blocked the full timeout, is still a lot of stall accumulated over
  "enough time"), and that same comment explicitly said to remove it
  ...
  code (`direction_cosine_from()`, `pitch_bend_baseline_x`/`_y`, the
  full cosine-delta compensation) shows had already happened, so this
  one's removal condition was already met by the prior implementer's
  own stated criteria, not a new judgment call.
  **All five fixed the same way as every other entry in this class**:
  the printf() and any comment that existed solely to justify keeping
  it around came out; comments explaining actual surrounding LOGIC (not
  the print) were kept where present (e.g. touch.c's "MIDI note on/off
  are owned by expression.c" note, expression.c's "released without a
  real press -- no note was ever sent" line). Two now-unused includes
  (`pico/time.h`, `stdio.h`) came out of `touch.c` along with its print,
  since nothing else in that file used either. **Left alone, deliberately,
  after the same review**: `expression.c`'s MPE-channel-steal print
  (only fires when all 15 Member Channels are simultaneously in use --
  a real edge case, not a per-note or per-scan hot path, even with the
  4-lane sequencer now holding channels of its own); every pitch-bend-
  enabled/sensitivity/mute toggle print (fires once per deliberate menu
  action, not per frame); `standby.c`'s wake-reason prints (fire once
  per standby-wake transition, not continuously); `expression_control.c`'s
  prints (explicit menu navigation, not a hot path); `op_mode.c`'s
  mode/scale/pattern-change prints (discrete user-initiated transitions,
  same category); and everything in `diagnostics/calibration.c` (every
  print there is gated behind `getchar_timeout_us(0)` actually returning
  a typed character -- silent, by construction, unless someone is typing
  into an open terminal right now).
